...
见页面底部 **差距面板**（数据来自 `src/data/backendGaps.ts`，随操作更新 API 日志）。

### 一句话结论

- **DBX 式「文件工作区」MVP（浏览/CRUD/回收站/简单编辑）**：后端 **基本满足**。
- **生产级对齐 Databricks**：需补 **大文件 STS**、**CORS/BFF**、**全局搜索**、**t_file 业务树**、**ACL** 等（见差距面板 P0/P1）。
...
src/data/backendGaps.ts       # 后端差距数据（差距面板）
```
